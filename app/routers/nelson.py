...
@router.post("/nelson", response_model=SuccessResponse | ErrorResponse)
async def nelson(data: TextRequest, llm: UnifiedLLMService = Depends(get_llm)):
    try:
        result = await llm.nelson(data.text)
